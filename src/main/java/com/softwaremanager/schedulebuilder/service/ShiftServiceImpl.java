...
public class ShiftServiceImpl implements ShiftService {
   ...
   public Shift addShiftToEmployee(Long shiftId, Long employeeId) {
      ...
      if (isShiftAlreadyAssignedToThisEmployee(employeeId, shiftToAdd))
         throw new EmployeeAlreadyHaveThisShiftException(employeeId);
   ...
   public void checkForDoubleItem(Shift shift) {
      for (Shift shiftSaved : getAllShifts()) {
         if (shiftSaved.getEndTime().equals(shift.getEndTime())
               && shiftSaved.getStartTime().equals(shift.getStartTime())) {
   ...
   public void deleteShftAssociatedWithEmployee(Long shiftId, Long employeeId) {
      ...
      if (!employee.getShifts().contains(shiftToDeleteFromEmployee)
            && !shiftToDeleteFromEmployee.getEmployees().contains(employee)) {
      ...
      updateShift(shiftToDeleteFromEmployee.getStartTime(), shiftToDeleteFromEmployee.getEndTime(),
            shiftToDeleteFromEmployee.getDate(), shiftId);
   ...
   // LocalDate startWeek = today.minusDays(today.getDayOfWeek().getValue() -
